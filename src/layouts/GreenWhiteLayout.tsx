...
const GreenWhiteLayout = ({ children }: PropsWithChildren) => {
  return (
    <div className="w-screen h-screen bg-white dark:bg-[#272A2E] relative">
      ...
      <div className="absolute top-2 sm:top-5 left-[10px] bg-[white] dark:bg-[#40424A] shadow dark:shadow-lg w-[calc(100%-35px)] sm:w-[calc(100%-64px)] h-[calc(100%-35px)] sm:h-[calc(100%-64px)] rounded-2xl m-2 sm:m-6 flex justify-center">
        {children}
      </div>
    </div>
  );
